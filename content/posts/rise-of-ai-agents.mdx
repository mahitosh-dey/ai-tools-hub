...
faqs:
  ...
  - q: "Is a top-tier ChatGPT plan worth paying for just to get agent features?"
    a: "Be careful with that reasoning, because the agent products that justified the upgrade keep being retired. Operator shut down on 31 August 2025. ChatGPT agent, which replaced it, was removed in early August 2026 with no deprecation notice. Atlas stopped working on 9 August 2026. Before paying for a higher tier to get agent capability, check what is actually shipping on the day you subscribe, and assume it is a feature that can be withdrawn rather than a product you are buying. Three withdrawals in this one product line since 2025 is the track record to price in."
...
A language model as the brain. This is what does the reasoning, figuring out what
to do next based on what has happened so far. Agents run on the frontier models from
OpenAI, Anthropic and Google. I am deliberately not naming versions: the original
draft of this post named three specific models and all three were superseded within
about three months. Check the vendor's own model page for what is current. The choice
shapes the personality, the reliability and the cost.
...
## This post recommended a tool that had shut down nine months earlier

When this page went live in June 2026 it told you to use OpenAI Operator. Three
separate times, including a closing line telling you to go and try it.

Operator shut down on **31 August 2025**. It had already been gone for nine months
when I published, and the recommendation stayed up for three months after that.

The replacement did not survive either. ChatGPT agent, which absorbed Operator's
browser control, was removed from ChatGPT in early August 2026 with no deprecation
notice. Atlas stopped working on 9 August 2026. OpenAI now folds browser agent work
into ChatGPT itself and into Codex, rather than shipping it as a product you can
point at.

I write a site about checking whether tools still exist before recommending them,
and this post did not check. The sections below are corrected. This note stays
rather than being quietly edited out, because the mistake tells you something
useful about this category: agent products get withdrawn faster than articles about
them get updated.
...
OpenAI's browser agent is the cautionary example above. Operator is gone, the
ChatGPT agent that replaced it is gone, and Atlas is gone. Browser-controlling work
now happens inside ChatGPT and Codex instead. If you want an agent that can find a
flight and book it, check what OpenAI actually ships on the day you subscribe, and
treat it as a feature that can be withdrawn rather than a product you own. That has
happened three times in this line since 2025. The [ChatGPT Plus review](/blog/chatgpt-plus-review)
covers what the tiers include.
...
Research and summarization. Give an agent a topic and it will search the web, read through sources, pull out the relevant information, and write you a structured summary. Not perfect, but significantly faster than doing it yourself. Perplexity's Deep Research and the deep research modes in ChatGPT and Claude are examples of this in production.

Code generation and testing. GitHub retired Copilot Workspace in May 2025, and that
work now happens through Copilot's agent mode in the editor, its Cloud Agent, and the
Copilot CLI. Cloud Agent is the interesting one: you assign a task and it plans and
executes on its own, and you can hand the job to Copilot, to Claude, or to OpenAI
Codex. The frontier models can all run code in a sandbox, check whether it works, and
fix it when it does not. If you write software for a living, [how to use Claude AI for coding](/blog/how-to-use-claude-ai-for-coding)
walks through how this actually feels in practice.
...
Complex multi-step tasks are still unreliable. An agent handling a 3-step task is reasonably reliable. A 20-step task with decisions at each step, much less so. The longer the chain, the more chances for something to go wrong, and the arithmetic
is unforgiving: a step that succeeds 95 percent of the time succeeds across twenty
steps only 36 percent of the time.

An earlier version of this paragraph gave precise figures for 3, 15 and 30 steps and
introduced them with the word "empirically". I went looking for the source while
rechecking this post and could not find one, so they are gone.

What I can point at instead is a February 2026 paper, Towards a Science of AI Agent
Reliability (arXiv 2602.16666, ICML 2026), which evaluated agentic models across two
benchmarks and found that recent capability gains produced only small improvements in
reliability. Accuracy and reliability come apart.

I am not quoting a step-by-step reliability curve to replace the numbers I removed.
I looked for one. The figures in circulation trace back to blog summaries rather than
to a study, and swapping unsourced numbers for differently unsourced numbers would not
be an improvement.
...
For personal automation, check what your provider actually ships today before
committing to anything. This is the category where products have been withdrawn
fastest. Start with something small and reversible rather than building a routine
around a feature that may not exist next quarter.
...
Published June 2026. Last rechecked and corrected on 22 September 2026, when three
of the products named here turned out to be discontinued. The agent landscape moves
faster than articles about it get updated, which this page is now a standing example
of. Check any product named here before you pay for it.
