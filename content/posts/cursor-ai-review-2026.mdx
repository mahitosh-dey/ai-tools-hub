...
Who owns it changed this summer. SpaceX agreed to buy Anysphere for $60 billion in an all-stock deal announced on June 16, 2026, expected to close in Q3 pending regulatory approval, and reported as the largest acquisition of a venture-backed startup on record. It follows SpaceX's merger with xAI in February, which is why Grok 4.5 now sits alongside Composer as a first-party Cursor model. For most solo developers this changes nothing about the editor today. If you work somewhere with a view on which companies your source code gets sent to, it's a question worth asking before you standardise a team on Cursor.
...
Copilot is the safer enterprise pick because of Microsoft's compliance story and its $10 price, though that $10 now includes $15 of metered credits rather than unlimited chat, which I unpack properly in [Cursor vs GitHub Copilot](/blog/cursor-vs-github-copilot). Windsurf is Cursor's closest direct competitor and slightly cheaper, but its Cascade agent still feels one release behind Cursor's Composer. Claude Code (Anthropic's terminal tool) isn't an IDE at all, but it's the best agentic tool for long-running background work.
